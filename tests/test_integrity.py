...
class IntegrityTests(unittest.TestCase):
    ...
    def valid_transaction_state(self):
        return {"schema_version":2,"project":"A","goal":"B","status":"active","constraints":[],"decisions":[],"next_action":None,"updated_at":"2026-09-25T00:00:00Z","revision":1}
    ...
    def test_pre_checksum_transaction_remains_readable(self):
        with tempfile.TemporaryDirectory() as d:
            root=Path(d); self.setup(root)
            tx={"txid":"legacy","state":self.valid_transaction_state(),"event":{"type":"note","message":"ok"}}
    ...
    def test_clean_aggregate_audit_is_valid_and_non_mutating(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(json.loads(r.stdout), {"schema_version":1,"valid":True,"first_break":None,"state":{"schema_version":1,"valid":True,"checked":1,"first_break":None},"events":{"schema_version":1,"valid":True,"checked":1,"first_break":None},"transaction":{"schema_version":1,"valid":True,"checked":0,"first_break":None}})
    ...
    def test_aggregate_audit_first_break_summary_is_deterministic(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            material={"txid":"x","state":self.valid_transaction_state(),"event":{"type":"note","message":"ok"}}
    ...
    def test_transaction_state_schema_tampering_is_rejected(self):
        with tempfile.TemporaryDirectory() as d:
            root=Path(d); self.setup(root)
            state=self.valid_transaction_state(); state["revision"]=-1
            material={"txid":"x","state":state,"event":{"type":"note","message":"ok"}}
            tx=dict(material); tx["checksum"]=__import__("hashlib").sha256(json.dumps(material, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()
            journal=root/"continuity/transaction.json"; journal.write_text(json.dumps(tx), encoding="utf-8")
            before=journal.read_text(encoding="utf-8")
            first=self.execute(root, "--audit-transaction"); second=self.execute(root, "--audit-transaction")
            self.assertNotEqual(first.returncode, 0); self.assertEqual(first.stdout, second.stdout); self.assertEqual(first.stderr, second.stderr)
            self.assertEqual(json.loads(first.stdout)["first_break"]["reason"], "transaction state schema v2 requires non-negative integer revision")
            self.assertEqual(before, journal.read_text(encoding="utf-8"))

    def test_event_chain_tampering_is_rejected(self):
        with tempfile.TemporaryDirectory() as d:
            root=Path(d); self.setup(root)
            row={"type":"note","message":"ok","prev_hash":"GENESIS"}; row["chain_hash"]="bad"
    ...
    def test_event_hash_fields_must_be_strings(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            (root/"continuity/events.jsonl").write_text(json.dumps(row)+"\n", encoding="utf-8")
            r=self.execute(root, "--audit"); self.assertNotEqual(r.returncode, 0); self.assertEqual(json.loads(r.stdout)["first_break"]["reason"], "event hash fields must be strings")
    ...
    def test_event_chain_hash_requires_lowercase_hex(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            (root/"continuity/events.jsonl").write_text(json.dumps(row)+"\n", encoding="utf-8")
            r=self.execute(root, "--audit"); self.assertNotEqual(r.returncode, 0); self.assertEqual(json.loads(r.stdout)["first_break"]["reason"], "event chain_hash must be 64 lowercase hex characters")

    def test_orphaned_event_chain_is_rejected(self):
        with tempfile.TemporaryDirectory() as d:
            root=Path(d); self.setup(root)
            row={"type":"note","message":"ok","prev_hash":"orphan"}
            material=dict(row); row["chain_hash"] = __import__("hashlib").sha256(json.dumps(material, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()
            (root/"continuity/events.jsonl").write_text(json.dumps(row)+"\n", encoding="utf-8")
            r=self.execute(root, "--audit"); self.assertNotEqual(r.returncode, 0); self.assertEqual(json.loads(r.stdout)["first_break"]["reason"], "event chain does not start at GENESIS")

    def test_audit_reports_first_broken_line_without_mutation(self):
        with tempfile.TemporaryDirectory() as d:
            root=Path(d); self.setup(root)
            row={"type":"note","message":"ok","prev_hash":"GENESIS"}; row["chain_hash"]="bad"
            events=root/"continuity/events.jsonl"; events.write_text(json.dumps(row)+"\n", encoding="utf-8"); before=events.read_text(encoding="utf-8")
            r=self.execute(root, "--audit"); self.assertEqual(json.loads(r.stdout)["valid"], False); self.assertIn('"line":1', r.stdout); self.assertEqual(before, events.read_text(encoding="utf-8"))

    def test_transaction_audit_reports_stable_checksum_error(self):
        with tempfile.TemporaryDirectory() as d:
            root=Path(d); self.setup(root)
            material={"txid":"x","state":self.valid_transaction_state(),"event":{"type":"note","message":"ok"}}; tx=dict(material); tx["checksum"]="0"*64
            journal=root/"continuity/transaction.json"; journal.write_text(json.dumps(tx), encoding="utf-8"); before=journal.read_text(encoding="utf-8")
            r=self.execute(root, "--audit-transaction"); self.assertNotEqual(r.returncode, 0)
    ...
    def test_aggregate_audit_reports_both_domains_without_mutation(self):
        with tempfile.TemporaryDirectory() as d:
            root=Path(d); self.setup(root)
            row={"type":"note","message":"ok","prev_hash":"GENESIS","chain_hash":"bad"}; events=root/"continuity/events.jsonl"; events.write_text(json.dumps(row)+"\n", encoding="utf-8")
            material={"txid":"x","state":self.valid_transaction_state(),"event":{"type":"note","message":"ok"}}; tx=dict(material); tx["checksum"]="0"*64
            ...
            r=self.execute(root, "--audit-all"); payload=json.loads(r.stdout)
            self.assertFalse(payload["valid"]); self.assertEqual(payload["first_break"], {"scope":"events","line":1,"reason":"event chain_hash must be 64 lowercase hex characters"})
    ...
    def test_non_object_event_record_is_rejected_deterministically(self):
        with tempfile.TemporaryDirectory() as d:
            root=Path(d); self.setup(root); (root/"continuity/events.jsonl").write_text("[]\n", encoding="utf-8")
            r=self.execute(root, "--audit"); self.assertNotEqual(r.returncode, 0); self.assertEqual(json.loads(r.stdout)["first_break"]["reason"], "event record must be an object")

    def test_non_object_transaction_is_rejected_deterministically(self):
        with tempfile.TemporaryDirectory() as d:
            root=Path(d); self.setup(root); (root/"continuity/transaction.json").write_text("[]\n", encoding="utf-8")
            r=self.execute(root, "--audit-transaction"); self.assertNotEqual(r.returncode, 0); self.assertEqual(json.loads(r.stdout)["first_break"]["reason"], "transaction journal must be an object")
