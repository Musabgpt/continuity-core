...
def init(args):
    with project_lock():
        ...
        s={"schema_version":LATEST_SCHEMA,"project":args.project,"goal":args.goal,"status":"active","constraints":[],"decisions":[],"next_action":None,"revision":0}; save_state_unlocked(s); append_event("success","Continuity state initialized.")
...
def set_next(args):
    with project_lock():
        s=load_state_unlocked(); require_latest(s); require_revision(s,args.expect_revision); s["next_action"]=args.action; s["revision"]+=1; transact_unlocked(s,"note","Next action set: "+args.action)
def validate(_):
    errors=[]
    with project_lock():
        try:
...
def migrate(_):
    with project_lock():
        ...
        if v==1: s["schema_version"]=2; s["revision"]=0; transact_unlocked(s,"success","Migrated state schema from v1 to v2.","v2 adds a monotonic revision counter for stale-state detection."); print("Migrated schema_version 1 -> 2"); return 0
    return 1
def handoff_payload():
    with project_lock():
        if TXN.exists():
