...
class RecoveryLockingTests(unittest.TestCase):
    def setup(self, root):
        ...
        r=subprocess.run([sys.executable,str(root/"continuity.py"),"init","--project","A","--goal","B"],text=True,capture_output=True,cwd=root)
    ...
    def test_recovers_crash_after_journal_before_event(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            r=subprocess.run([sys.executable,str(root/"continuity.py"),"validate"],text=True,capture_output=True,cwd=root)
            self.assertNotEqual(r.returncode,0)
            self.assertIn("pending transaction journal",r.stdout+r.stderr)
            self.assertTrue((c/"transaction.json").exists())
    ...
    def test_recovers_crash_after_event_without_duplicate(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            (c/"transaction.json").write_text(json.dumps({"txid":txid,"state":state,"event":row}))
            r=subprocess.run([sys.executable,str(root/"continuity.py"),"handoff","--format","json"],text=True,capture_output=True,cwd=root)
            self.assertNotEqual(r.returncode,0)
            self.assertIn("Pending transaction journal",r.stdout+r.stderr)
    ...
    def test_concurrent_expected_revision_allows_exactly_one_writer(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            a=subprocess.Popen(cmd+["writer-a","--expect-revision","0"],text=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE,cwd=root)
            b=subprocess.Popen(cmd+["writer-b","--expect-revision","0"],text=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE,cwd=root)
