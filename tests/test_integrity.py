...
class IntegrityTests(unittest.TestCase):
    ...
    def test_legacy_events_remain_readable(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            r=self.run(root); self.assertEqual(r.returncode, 0, r.stdout+r.stderr)
    ...
    def test_transaction_checksum_tampering_is_rejected(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            r=self.run(root); self.assertNotEqual(r.returncode, 0); self.assertIn("checksum mismatch", r.stdout)
    ...
    def test_event_chain_tampering_is_rejected(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            r=self.run(root); self.assertNotEqual(r.returncode, 0); self.assertIn("hash mismatch", r.stdout)
